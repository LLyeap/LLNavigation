...
@property (nonatomic, retain) UIImage *btnRightImage;
 ...
 */
- (instancetype)initWithFrame:(CGRect)frame ItemArray:(NSArray *)itemArray btnRightImage:(UIImage *)btnRightImage itemClickBlock:(void (^)(NSInteger))itemBlock btnRightBlock:(void (^)(UIButton *))btnRightBlock {
    self = [super initWithFrame:frame];
    if (self) {
        self.itemArray = itemArray;
        self.btnRightImage = btnRightImage;
        [self createView];
...
- (void)createView {
    ...
    [btnRight setImage:_btnRightImage forState:UIControlStateNormal];
...
//        带动画出Bug
//        [_scrollView setContentOffset:CGPointMake(currentBtn.frame.origin.x + currentBtn.frame.size.width/2 - SCREENWIDTH/2, 0) animated:YES];
